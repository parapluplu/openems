...
class EmsClient:
    ...
    def get_active_power(self) -> int:
        ...
        return self.read_channel("ess0", "ActivePower").value
    ...
    def get_soc(self) -> int:
        # TODO configure ess id dynamically
        return self.read_channel("ess0", "Soc").value
    ...
    def wait_for_soc(
        ...
        if soc < desired_soc:
            verify_soc = lambda: self.get_soc() >= desired_soc
        ...
        if exact_stop:
            assert desired_soc == self.get_soc()
