...
def pytest_configure(config):
    config.addinivalue_line(
        "markers",
        f"{HARDWARE_MARKER}: mark a test that it only runs in a setup with real hardware",
    )


def pytest_collection_modifyitems(config, items):
    if config.getoption("--run-on-hardware"):
        # run all tests TODO evaluate if there are simulation only tests
        return
    skip_hardware = pytest.mark.skip(reason="Don't run hardware tests. Add --run-on-hardware to execute.")
    for item in items:
        if HARDWARE_MARKER in item.keywords:
